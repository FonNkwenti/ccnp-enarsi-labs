#!/usr/bin/env python3
"""
Solution Restoration — DMVPN Lab 02: Capstone I — Full Configuration Mastery

Reads per-device configs from solutions/ and pushes them to all devices,
returning the lab to the known-good state after fault injection.
...
# parents[0] = fault-injection, parents[1] = scripts, parents[2] = lab-02-capstone-config
CONFIGS_DIR = Path(__file__).resolve().parents[2] / "solutions"
...
def restore_device(host: str, ports: dict, name: str, *, reset: bool) -> bool:
    ...
    cfg_file = CONFIGS_DIR / f"{name}.cfg"
