#!/usr/bin/env python3
"""
Fault Injection: Scenario 01 — R2 Default Route Removed

Target:     R2 (GigabitEthernet0/0 — underlay link to R4)
Injects:    Removes R2's static default route (ip route 0.0.0.0 0.0.0.0 198.51.100.2)
Fault Type: Missing Static Route (Underlay Reachability)

Result:     R2 loses all connectivity outside its local subnet. NHRP registration
            to the hub fails; OSPF adjacency is never established. show dmvpn on R1
            shows R2 entry absent or expired.
...
# Pre-flight: check routing table for the default route on R2.
PREFLIGHT_CMD = "show ip route 0.0.0.0"
# Present only when the fault is already injected (route removed).
PREFLIGHT_FAULT_MARKER = "% Network not in table"
# Present only in the known-good state (default route installed).
PREFLIGHT_SOLUTION_MARKER = "198.51.100.2"
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 01 — Ticket 1")
    print("=" * 60)
