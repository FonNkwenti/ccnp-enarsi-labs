#!/usr/bin/env python3
"""
Fault Injection: Scenario 02 — R3 NHRP Network-ID Mismatch

Target:     R3 (Tunnel0 — NHRP client registration toward hub R1)
Injects:    Changes R3's NHRP network-id from 100 to 200 (non-matching value)
Fault Type: NHRP Parameter Mismatch (Network-ID)

Result:     R3 fails to register with the hub NHS. R1's show dmvpn shows only
            one spoke (R2). R3 shows no dynamic NHRP entry for the hub. OSPF
            adjacency between R1 and R3 never forms.
...
# Pre-flight: inspect the Tunnel0 running-config on R3.
PREFLIGHT_CMD = "show running-config interface Tunnel0"
# Present only after the fault is injected (wrong network-id).
PREFLIGHT_FAULT_MARKER = "ip nhrp network-id 200"
# Present only in the known-good state (correct network-id).
PREFLIGHT_SOLUTION_MARKER = "ip nhrp network-id 100"
...
def main() -> int:
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 02 — Ticket 2")
    print("=" * 60)
